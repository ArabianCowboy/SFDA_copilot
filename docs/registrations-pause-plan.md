...
called it as well, until that file was deleted whole on 2026-09-09 alongside the `/auth/login`
retirement — both of its tests signed in through the route being retired, and both asserted a
token shape the code had already stopped returning.)
...
| Rate limit on the migrated route       | New `signup_bp`, limited in `_register_routes`                                            | Signup sends mail. `auth.py:11–23` documents why a decorator at import registers nowhere, and why `recover_bp` exists as its own blueprint rather than limiting all of `auth_bp`. That comment justified itself at the time with "a 5/minute ceiling on logout would be wrong" — a sentence removed on 2026-09-09 because it described an exemption that never existed: `auth_bp` carries no limit, so logout had been running at the global defaults all along. |
