...
- [Deletion step-up blinds GoTrue's own per-IP rate limiter](#deletion-step-up-blinds-gotrues-own-per-ip-rate-limiter) — throttle built AND APPLIED 2026-09-19 (`20260918234736_step_up_attempts`); not live yet because the feature switch is off.
...
- [Account deletion (Spec 4)](#account-deletion-spec-4--blocked-on-a-product-decision-not-on-engineering) — decision closed 2026-09-18 (yes, 30-day grace); built, gate-green, and **eleven of fifteen migrations applied 2026-09-19**. Held on: the code deploy (`03`-`05`), one real deletion (`13`), and the feature switch. See `supabase/pending/README.md`.
...
- [Does "disabled" freeze an account's own profile edits?](#does-disabled-freeze-an-accounts-own-profile-edits-or-only-its-use-of-the-product) — decided 2026-09-18 (freeze everything but consent withdrawal); built and gate-green. The two consent RPCs are applied; the freeze itself (`03`-`05`) waits on the code deploy.
