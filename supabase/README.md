...
supabase/
  ...
  pending/        migrations written but not yet applied, named NN_<name>.sql
...
### `supabase/pending/` — migrations written but not yet applied

When a slice is built without touching the live database (no `apply_migration`, no
`db push`), its migrations cannot carry real versions yet — and inventing a
timestamp would fabricate the very ordering the filename rule exists to record.
So they wait in `supabase/pending/` under an ordinal name, `NN_<name>.sql`,
where `NN` (`01_`, `02_`, …) gives the intended APPLY order only. An ordinal
is deliberately not a version: two pending files never claim to know when
they will land, only which goes first.

Applying one is the filename rule's fourth step, unchanged: `apply_migration`
it, read the real version back from `list_migrations`, then `git mv` it into
