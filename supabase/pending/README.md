...
| —      | **flip `account_deletion_self_serve_enabled` to `true`, then verify** | **FLIPPED 2026-09-19** — gate met: timer installed, enabled and ticking, and `15` applied. **The round trip is still OUTSTANDING:** request a deletion on a throwaway account, watch the ledger row reach `pending`, cancel it, confirm the cancel lands. Until it passes, the `/privacy` promise is live but unverified |
