...
**Eleven of the fifteen were applied on 2026-09-19 and have moved to `../migrations/`. FOUR
remain here**, and each is held by something specific rather than by nerves:

- **`03`, `04`, `05` are held by the code deploy.** `03` revokes the consent column grants the
  currently-deployed browser still writes through, so applying it before the repointed toggle
  is live breaks consent withdrawal silently for every reader. `04` and `05` are the same
  problem mirrored: they freeze a disabled reader's writes while that reader still cannot
  reach the new withdrawal RPC, which is precisely the hole the carve-out exists to close.
- **`13` is parked** until one real deletion completes end to end — see below.
